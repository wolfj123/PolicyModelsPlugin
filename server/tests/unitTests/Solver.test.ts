...
import { languagesIds, getLanguageIdFromfile } from '../../src/Utils';
...
describe('Auto complete Tests', ()=>{
	...
	const createChangeParams = (fileName: string, folder:string, changeText: string, range: Range, changeLength: number): DidChangeTextDocumentParams => {
		let ans: DidChangeTextDocumentParams = {
			textDocument:{
				uri:uriCreator(fileName,folder),
				version:2
			},
			contentChanges:
			[
	...
	async function testInferrerExample() {
	...
	it ("autocomplete for File",async ()=> {
		// open 2 files
		let recFolder: string = "C:\\Ariel\\Final Project\\Project\\PolicyModelsPlugin\\server\\tests\\sample directory\\a\\Recursive-Sections"
		let recDGFile = "decision-graph.dg"
		let recPSFile: string  = "policy-space.pspace"
		let playFolder: string = "C:\\Ariel\\Final Project\\Project\\PolicyModelsPlugin\\server\\tests\\sample directory\\a\\a\\Find-Runs-playground";
		...
		let x = FS.readFileSync(path.join(recFolder,recDGFile).toString(),'utf-8');
